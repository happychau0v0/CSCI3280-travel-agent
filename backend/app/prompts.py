...
- submit_trip_form(destination?, origin?, start_date?, end_date?, transport?, party_size?, interests?) — pre-fill the trip planning form and auto-start planning when all required fields (destination, start_date, end_date, transport) are valid. If any required field is missing or invalid, the form is pre-filled for the user to review and submit. Use when the user says things like "plan a trip to Tokyo".
